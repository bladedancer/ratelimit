...
For every main commit, an image is pushed to [Dockerhub](https://hub.docker.com/r/bladedancer/ratelimit/tags?page=1&ordering=last_updated). There is currently no versioning (post v1.4.0) and tags are based on commit sha.
...
3. `e91321b` [commit](https://github.com/bladedancer/ratelimit/commit/e91321b10f1ad7691d0348e880bd75d0fca05758) deleted support for the legacy [ratelimit.proto](https://github.com/bladedancer/ratelimit/blob/0ded92a2af8261d43096eba4132e45b99a3b8b14/proto/ratelimit/ratelimit.proto).
...
The path to watch can be configured via the [settings](https://github.com/bladedancer/ratelimit/blob/master/src/settings/settings.go)
...
The [gRPC client](https://github.com/bladedancer/ratelimit/blob/master/src/client_cmd/main.go) will interact with ratelimit server and tell you if the requests are over limit.
...
redis cache again for the already over-the-limit keys. The local cache size can be configured via `LocalCacheSizeInBytes` in the [settings](https://github.com/bladedancer/ratelimit/blob/master/src/settings/settings.go).
